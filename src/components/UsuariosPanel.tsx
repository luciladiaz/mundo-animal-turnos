...
const FORM_VACIO = { nombre: "", email: "", password: "", rol: "secretaria" as "admin" | "secretaria" };
...
export default function UsuariosPanel() {
  ...
  async function cambiarRol(u: Usuario, rol: "admin" | "secretaria") {
    setError(null);
    const res = await fetch(`/api/usuarios/${u.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
    ...
    <section className="card-suave rounded-xl border border-humo-200 bg-white p-4">
      <div className="mb-3 flex items-center justify-between">
        <p className="text-xs text-humo-500">
          Cuentas con acceso al panel (ej: la secretaria que carga turnos). Solo un administrador puede crear o
          modificar usuarios.
        </p>
        {!mostrarForm && (
          <button
            onClick={() => setMostrarForm(true)}
            className="btn-primary w-fit rounded-lg px-3 py-1.5 text-sm"
          >
            + Nuevo usuario
          </button>
        )}
        ...
        <form onSubmit={crearUsuario} className="mb-4 flex flex-col gap-3 rounded-lg bg-humo-50 p-3">
          ...
          <label className="flex flex-col gap-1 text-sm">
            ...
            <input
              ...
              className="rounded-lg border border-humo-200 px-3 py-2 outline-none transition focus:border-[var(--color-primario)] focus:ring-2 focus:ring-mora-100"
              placeholder="Ej: maria.secretaria"
            />
          </label>
          <label className="flex flex-col gap-1 text-sm">
          ...
          <label className="flex flex-col gap-1 text-sm">
            <span className="text-humo-600">Rol</span>
            <select
              value={form.rol}
              onChange={(e) => setForm({ ...form, rol: e.target.value as "admin" | "secretaria" })}
              className="rounded-lg border border-humo-200 px-3 py-2 outline-none transition focus:border-[var(--color-primario)] focus:ring-2 focus:ring-mora-100"
            >
              <option value="secretaria">Secretaria (solo Dashboard y Turnos)</option>
            ...
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                <p className="font-medium text-humo-900">
                  {u.nombre} {!u.activo && <span className="text-xs font-normal text-humo-400">(desactivado)</span>}
                </p>
              ...
              <div className="flex flex-wrap items-center gap-2">
                <select
                  value={u.rol}
                  onChange={(e) => cambiarRol(u, e.target.value as "admin" | "secretaria")}
                  className="rounded-md border border-humo-200 px-2 py-1 text-xs"
                >
